"""Implementation of pole-residue dispersion fitter."""
...
def imag_resp_extrema_locs(poles: ArrayComplex1D, residues: ArrayComplex1D) -> ArrayFloat1D:
    """Locations of extrema of Im[resp] in the same unit as poles.

    Parameters
    ----------
    poles: ArrayComplex1D
        Complex-valued poles for the model.
    residues: ArrayComplex1D
    ...
    try:
        poles_a = np.array(poles)
        poles_c = np.array(residues)
...
def fit(
    ...
    Note
    ----
        The frequency-dependence of the complex-valued response function is described by:

        .. math::

            f(\\omega) = f_\\infty - \\sum_i
    ...
    ----------
    omega_data: ArrayFloat1D
        Angular frequency data in rad / sec.
    resp_data: ArrayComplex1D
        Complex-valued response function data to be fit.
    ...
    resp_inf : float, optional
        Value of resp_inf to use in fit. If None, then resp_inf is also fit.
        Note: fitting resp_inf is not guaranteed to yield a global optimum, so
        the result may occasionally be better with a fixed value of resp_inf.
    ...
    omega = np.real(omega_data) * scale_factor
    eps = resp_data
    eps_inf = resp_inf
