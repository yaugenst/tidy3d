"""Defines specification for the terminal mode solver."""
...
# from ....plugins.microwavepath_integrals import (
...
class TerminalSpec(Tidy3dBaseModel):
    ...
    voltage_integral: Optional[Union[VoltageIntegralTypes, tuple[VoltageIntegralTypes, ...]]] = (
        pd.Field(
            None,
    ...
    current_integral: Optional[Union[CurrentIntegralTypes, tuple[CurrentIntegralTypes, ...]]] = (
        pd.Field(
            None,
    ...
    def _create_current_paths(
    ...
    ) -> tuple[list[CurrentIntegralTypes], list[Shapely]]:
        ...
        merged_geos = TerminalSpec._get_isolated_conductors_as_shapely(mode_plane, structures)
        ...
        for shape in merged_geos:
            if not shapely_port_boundary.touches(shape):
                ...
                terminal_current = CurrentIntegralAxisAligned(
                ...
                current_integrals.append(terminal_current)
                print(box_snapped.bounds)
        for current_integral in current_integrals:
            assert not any(
                TerminalSpec._check_path_intersects_with_conductors(merged_geos, current_integral)
            ), "Cannot automate path setup."
        return current_integrals, merged_geos
